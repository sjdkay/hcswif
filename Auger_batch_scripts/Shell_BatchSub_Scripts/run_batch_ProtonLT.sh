...
while true; do
    ...
    case $yn in
		...
		batch="${USER}_${runNum}_ProtonLT_Job.txt"
		...
		### 03/03/21 - SK, note that this script needs fixing still (pathing of scripts it calls has not been adjusted)
                echo "COMMAND:/group/c-pionlt/USERS/${USER}/hallc_replay_lt/hcswif/Analysis_Scripts/ProtonLT.sh ${runNum} ${MAXEVENTS}"  >> ${batch}
